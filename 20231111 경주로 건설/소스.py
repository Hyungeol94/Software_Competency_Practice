...
def solution(board):
    ...
    while pq:
        ...
        for dx, dy, path_dir in zip(dxs, dys, path_dirs):
            next_row = row + dy
            next_col = col + dx
            cost = 100 if path_dir == current_path_dir else 600
            if isValid(next_row, next_col, n) and not isVisited(visited, next_row, next_col) and not board[next_row][
                                                                                                         next_col] == 1:
                new_visited = copy(visited)
                markAsVisited(new_visited, next_row, next_col)
                heapq.heappush(pq, [current_cost + cost, path_dir, new_visited, next_row, next_col])